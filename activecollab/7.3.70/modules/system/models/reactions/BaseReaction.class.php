...
abstract class BaseReaction extends ApplicationObject implements IWhoCanSeeThis, ActiveCollab\Foundation\Urls\Router\Context\RoutingContextInterface, IChild, ICreatedOn, ICreatedBy
{
    use IWhoCanSeeThisImplementation;
    use IChildImplementation;
    use ICreatedOnImplementation;
    use ICreatedByImplementation;
    const MODEL_NAME = 'Reaction';
    const MANAGER_NAME = 'Reactions';

    protected string $table_name = 'reactions';
    ...
    {
        if ($singular) {
            return $underscore ? 'reaction' : 'Reaction';
        } else {
            return $underscore ? 'reactions' : 'Reactions';
        }
    }
