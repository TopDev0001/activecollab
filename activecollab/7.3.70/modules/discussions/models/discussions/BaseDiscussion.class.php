...
abstract class BaseDiscussion extends ApplicationObject implements ISubscriptions, IIncomingMail, IComments, IAttachments, IHistory, IAccessLog, \Angie\Search\SearchItem\SearchItemInterface, ITrash, IActivityLog, IHiddenFromClients, IProjectElement, IWhoCanSeeThis, ActiveCollab\Foundation\Urls\Router\Context\RoutingContextInterface, IBody, ICreatedOn, ICreatedBy, IUpdatedOn, IUpdatedBy
{
    ...
    const MANAGER_NAME = 'Discussions';

    protected string $table_name = 'discussions';
    ...
    {
        if ($singular) {
            return $underscore ? 'discussion' : 'Discussion';
        } else {
            return $underscore ? 'discussions' : 'Discussions';
        }
    }
