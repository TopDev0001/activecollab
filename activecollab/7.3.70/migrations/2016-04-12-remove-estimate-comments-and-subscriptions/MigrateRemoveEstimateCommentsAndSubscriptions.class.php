 ...
 */
class MigrateRemoveEstimateCommentsAndSubscriptions extends AngieModelMigration
{
    public function up()
    {
        if ($comment_ids = $this->executeFirstCell('SELECT id FROM comments WHERE parent_type = ?', Estimate::class)) {
            AngieApplication::log()->info(self::class . ' migration found {estimate_comments_num} estimate comments in account {account_id}', [
                'estimate_comments_num' => count($comment_ids),
            ]);
        ...
        $this->execute('DELETE FROM comments WHERE parent_type = ?', Estimate::class);
        $this->execute('DELETE FROM subscriptions WHERE parent_type = ?', Estimate::class);
    }
}
