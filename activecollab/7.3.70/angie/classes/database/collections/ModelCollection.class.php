...
class ModelCollection extends DataObjectCollection
{
    ...
    public function __construct(string $name, string $model_name)
    {
    ...
    public function getTimestampHash($timestamp_field)
    {
        ...
        if ($this->count() > 0) {
            if ($join_expression = $this->getJoinExpression()) {
                return sha1(
                    (string) DB::executeFirstCell(
                        sprintf(
                            "SELECT GROUP_CONCAT(%s.%s ORDER BY %s.id SEPARATOR ',') AS 'timestamp_hash' FROM %s %s %s",
                            $table_name,
                            $timestamp_field,
                            $table_name,
                            $table_name,
            ...
            return sha1(
                (string) DB::executeFirstCell(
                    sprintf(
                        "SELECT GROUP_CONCAT(%s.%s ORDER BY id SEPARATOR ',') AS 'timestamp_hash' FROM %s %s",
                        $table_name,
                        $timestamp_field,
                        $table_name,
                        $conditions,
     ...
     * @return string|null
     */
    protected function getJoinExpression()
    {
        $expression = '';
